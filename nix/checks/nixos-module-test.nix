...
pkgs.testers.nixosTest {
  ...
  testScript = ''
    ...
    with subtest("OAuth: Service starts with bind-mount"):
        oauth.wait_for_unit("clawdbot-gateway.service", timeout=60)

    with subtest("OAuth: Can read credentials via bind-mount"):
        # The service should be able to read the .claude directory
        oauth.succeed(
            "nsenter -t $(systemctl show -p MainPID --value clawdbot-gateway.service) -m "
            "cat /var/lib/clawdbot/.claude/credentials.json | grep -q fake-oauth-token"
        )
    ...
    oauth.log(oauth.succeed("systemctl status clawdbot-gateway.service"))
  '';
}
